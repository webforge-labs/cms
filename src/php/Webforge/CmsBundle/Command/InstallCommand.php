...
class InstallCommand extends ContainerAwareCommand {
  ...
  protected function config() {
    $this->target = Dir::factoryTS($this->askStringValue('Provide absolute path to the target directory', $this->input->getArgument('target')));
    ...
    $this->config = (object) [
      ...
      "project.name"=>$this->askStringValue('Name des Projektes', $name = $this->input->getArgument('project-name') ?: str_replace('-', ' ', $this->target->getName())),
      "project.nicename"=>$nicename = $this->askStringValue('technischer Name des Projektes', $this->input->getOption('project-nicename') ?: S::camelCaseToDash($name)),
      'project.bundle_name'=> $bundleName = $this->askStringValue('Name des Symfony Bundles (in camelCase)', $this->input->getOption('bundle-name') ?: S::dashToCamelCase($nicename).'Bundle' ),
  ...
  protected function configureSymfonyParameters($input, $output) {
    ...
    $parameters = [
      ...
      'database_port'=>NULL,
      'database_name'=> $name = $this->askStringValue('value for database_name', $this->config->{'project.nicename'}),
      'database_user'=> $this->askStringValue('value for database_user', $name),
      'database_password'=> $this->askStringValue('value for database_password', mb_substr($this->generateNewSecret(), 0, 7)),
    ...
    $sql[] = sprintf("CREATE USER '%s'@'%s' IDENTIFIED BY '%s';", $parameters['database_user'], $parameters['database_host'], $parameters['database_password']);
    $sql[] = sprintf("GRANT USAGE ON *.* TO '%s'@'%s' IDENTIFIED BY '%s' WITH MAX_QUERIES_PER_HOUR 0 MAX_CONNECTIONS_PER_HOUR 0 MAX_UPDATES_PER_HOUR 0 MAX_USER_CONNECTIONS 0;", $parameters['database_user'], $parameters['database_host'], $parameters['database_password']);
    $sql[] = sprintf("GRANT ALL PRIVILEGES ON `%s`.* TO '%s'@'%s';", $parameters['database_name'], $parameters['database_user'], $parameters['database_host']);
  ...
  protected function exec($commandline, Dir $workdir = NULL) {
    if(!isset($workdir)) $workdir = $this->target;
